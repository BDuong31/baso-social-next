  ...
  return (
    <AlertDialog open={isOpen} onOpenChange={onClose}>
      <AlertDialogContent className="max-w-md">
        <AlertDialogHeader>
          <AlertDialogTitle>{t('update cover picture')}</AlertDialogTitle>
        </AlertDialogHeader>
        ...
        <div className="flex flex-col items-center gap-6 py-4">
          ...
          <div className="text-center text-sm dark:text-tertiary text-surface">
            {previewUrl ? (
              <p>{t('click the image to choose a different photo')}</p>
            ) : (
              <p>{t('click the image to choose a photo')}</p>
            )}
          </div>
        </div>
