...
const BrightnessSlider: React.FC<BrightnessSliderProps> = ({
  ...
  return (
    ...
    >
      {/* Slider Track */}
      <div
        className="w-full h-full dark:bg-gray-200 bg-gray-700 rounded-xl overflow-hidden"
        style={{
          position: 'relative',
          background: `linear-gradient(to right, ${theme === "dark" ? "#F8F8F81A" : "#0707071A"} ${brightness}%, ${theme === "dark" ? "#F8F8F80D" : "#0707070D"} ${brightness}%)`,
        }}
      />
      ...
      <span className="absolute left-7 top-0 bottom-0 inline-flex justify-start items-center transform -translate-x-1/2 text-sm font-semibold ">
        <Typography level="captionsm" className=" dark:text-primary text-surface-3">
          {brightness}%
        </Typography>
      </span>
    </div>
