...
export const NotificationItem: React.FC<NItemProps> = ({
  ...
  const regex = /liked your post|replied to your comment|followed you/;
  const content = notification.content.match(regex);
  const { t } = useTranslation();

  return (
    <li
      onClick={onRead}
      className="cursor-pointer relative dark:bg-neutral2-2 bg-neutral1-30 rounded-[20px] p-3 flex gap-2.5 items-center self-stretch"
    >
      <CircleAvatar
      ...
      <div className="flex flex-1 flex-col gap-3">
        <span className="inline-flex flex-wrap gap-1">
          <Typography
            level="base2r"
            className="line-clamp-2 font-bold dark:text-primary text-surface-3 opacity-80"
          >
        ...
        {notification.content && (
          <Typography level="base2r" className="dark:text-primary text-surface-3">
            {notification.sender.firstName} {notification.sender.lastName} {t(`${content}`)}
          </Typography>
        )}
        <Typography level="base2r" className="dark:text-tertiary text-surface">
          {timeAgo}
